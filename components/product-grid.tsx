    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {products.map((p) => (
        <div key={p.slug} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <Link href={`/produto/${p.slug}`} className="mt-2 block text-sm font-semibold hover:underline">
            {p.name}
          </Link>
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <Link className="text-sm font-semibold text-slate-900 hover:underline" href={`/produto/${p.slug}`}>
              Ver →
            </Link>
          </div>
